...
class CtpTdApi(TdApi):
    ...
    def onRspQryInstrument(self, data, error, n, last):
        """合约查询回报"""
        q_contract.put(data) #tianhm added for handle the contract info.
...
def DrConfigFileGenerator(instruments):
    # instruments 为主程序生成的 instrument 列表，
    import json

    Drfile = 'DR_setting.json'
    jf = open(Drfile, 'w')
    # 初始化配置文件数据
    drSetting = {}
    drSetting['tick'] = []
    ...
    if instruments:
        for inst in instruments:
            ...
            drSetting['tick'].append([inst, 'CTP'])
            drSetting['bar'].append([inst, 'CTP'])

    print drSetting
    # jf.write(json.loads(drSetting))
...
def test():
    ...
    while not q_contract.empty():
        contract = q_contract.get()

        # print contract['IsTrading'],contract['ExpireDate'],contract['InstrumentID'],contract['ExchangeID'],contract['InstrumentName'].decode('gbk'),contract['CreateDate'],\
        # contract['MaxMarketOrderVolume'],contract['MinMarketOrderVolume'],contract['ExchangeInstID'],contract['ProductID']
        '''
        {'IsTrading': 1, 'ExpireDate': '20190410', 'PositionDateType': '2', 'LongMarginRatio': 1.7976931348623157e+308,
         'StrikePrice': 3150.0, 'UnderlyingMultiple': 1.0, 'PositionType': '2', 'ProductClass': '2', 'MinSellVolume': 0,
         'InstrumentName': 'm1905-P-3150', 'ShortMarginRatio': 1.7976931348623157e+308, 'VolumeMultiple': 10,
         'MaxMarginSideAlgorithm': '0', 'DeliveryYear': 2019, 'CombinationType': '0', 'MinLimitOrderVolume': 1,
         'InstrumentID': 'm1905-P-3150', 'MaxLimitOrderVolume': 100, 'ExchangeID': 'DCE', 'CreateDate': '20180517',
         'MaxMarketOrderVolume': 100, 'OptionsType': '2', 'StartDelivDate': '20190410', 'DeliveryMonth': 5,
         'MinBuyVolume': 0, 'InstrumentCode': '', 'PriceTick': 0.5, 'InstLifePhase': '1',
         'ExchangeInstID': 'm1905-P-3150', 'MinMarketOrderVolume': 1, 'EndDelivDate': '20190410', 'ProductID': 'm_o',
         'OpenDate': '20180517', 'UnderlyingInstrID': 'm1905'}

         {'IsTrading': 1, 'ExpireDate': '20181114', 'PositionDateType': '2', 'LongMarginRatio': 0.05, 'StrikePrice': 0.0, 'UnderlyingMultiple': 1.0, 'PositionType': '2', 'ProductClass': '1', 'MinSellVolume': 0, 'InstrumentName': '\xb0\xd7\xc9\xb0\xcc\xc7811', 'ShortMarginRatio': 0.05, 'VolumeMultiple': 10, 'MaxMarginSideAlgorithm': '0', 'DeliveryYear': 2018, 'CombinationType': '0', 'MinLimitOrderVolume': 1, 'InstrumentID': 'SR811', 'MaxLimitOrderVolume': 1000, 'ExchangeID': 'CZCE', 'CreateDate': '20170516', 'MaxMarketOrderVolume': 200, 'OptionsType': '\x00', 'StartDelivDate': '20181114', 'DeliveryMonth': 11, 'MinBuyVolume': 0, 'InstrumentCode': '', 'PriceTick': 1.0, 'InstLifePhase': '1', 'ExchangeInstID': 'SR811', 'MinMarketOrderVolume': 1, 'EndDelivDate': '20181114', 'ProductID': 'SR', 'OpenDate': '20170516', 'UnderlyingInstrID': ''}

        {'IsTrading': 1, 'ExpireDate': '20181114', 'PositionDateType': '2', 'LongMarginRatio': 0.05, 'StrikePrice': 0.0,
         'UnderlyingMultiple': 1.0, 'PositionType': '2', 'ProductClass': '1', 'MinSellVolume': 0,
         'InstrumentName': '\xb0\xd7\xc9\xb0\xcc\xc7811', 'ShortMarginRatio': 0.05, 'VolumeMultiple': 10,
         'MaxMarginSideAlgorithm': '0', 'DeliveryYear': 2018, 'CombinationType': '0', 'MinLimitOrderVolume': 1,
         'InstrumentID': 'SR811', 'MaxLimitOrderVolume': 1000, 'ExchangeID': 'CZCE', 'CreateDate': '20170516',
         'MaxMarketOrderVolume': 200, 'OptionsType': '\x00', 'StartDelivDate': '20181114', 'DeliveryMonth': 11,
         'MinBuyVolume': 0, 'InstrumentCode': '', 'PriceTick': 1.0, 'InstLifePhase': '1', 'ExchangeInstID': 'SR811',
         'MinMarketOrderVolume': 1, 'EndDelivDate': '20181114', 'ProductID': 'SR', 'OpenDate': '20170516',
         'UnderlyingInstrID': ''}
        ...
        if contract['ProductClass'] == '1': #期货品种
            tmpproduct = contract['ProductID'] #产品分类ID
            tmpinst = contract['InstrumentID'] #合约ID
        

            if instruments_dict.has_key(tmpproduct): #如果产品类别已经存在，将合约插入
                instruments_dict[tmpproduct].append(tmpinst)
                
            else: #新增产品类别，然后将合约数据插入
                instruments_dict[tmpproduct]=[tmpinst]
            if contract['InstrumentID'] not in instruments:
                instruments.append(contract['InstrumentID'])

    DrConfigFileGenerator(instruments)
    inst = SortedList(instruments)
    tdate = str(datetime.now()).split(' ')[0]  #获取当前日期
    jfile = ''.join(['contractlist',tdate,'.json']) #生成文件名

    jf = open(jfile,'w')
    json.dump(instruments_dict,jf)
    jf.close()

    instruments_file = ''.join(['instruments', tdate, '.json'])
    jf1 = open(instruments_file, 'w')
    json.dump(instruments, jf1)
    jf1.close()
